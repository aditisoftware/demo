...
<!------------------------------------------- PRIVATE ------------------------------------------>
    ...
    <cffunction name="generateMessage" output="false" access="private" returntype="string">
		...
		<cfif arrayLen(msgStruct.message)>
			<cfsavecontent variable="local.retMsg"><cfoutput><div class="errorbox"><ul class="#msgStruct.type#"><cfloop from="1" to="#arrayLen(msgStruct.message)#" index="i"><li class="#msgStruct.type#">#msgStruct.message[i]#</li></cfloop></ul></div></cfoutput></cfsavecontent>
