<cfcomponent name="codeService">
	...
	<cffunction name="getComponents" access="public" output="false" returntype="array">
		...
		<cfdump var="#variables.config.generator.xmlChildren#"><cfdump var="#arguments.xmlTable.root.bean.paths.xmlChildren#"><cfabort>
		...
		<cfloop from="1" to="#arrayLen(variables.config.generator.xmlChildren)#" index="i">
			...
			<cfset arguments.xmlTable.root.bean.xmlAttributes.path = arguments.xmlTable.root.bean.paths.xmlChildren[i].xmlname>
			...
			<cfset objPage = createObject("component","model.code.generatedPage").init(name,template,content,thisRootPath) />
