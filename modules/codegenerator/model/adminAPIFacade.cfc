<cfcomponent name="adminAPIFacade" output="false" hint="a wrapper for admiapi functions">
	<cffunction name="init" access="public" output="false" returntype="adminAPIFacade">
		...
		<cfset variables.administrator = createObject("component","cfide.adminapi.administrator").login(arguments.moduleSetting.codegenerator.setting.adminPassword) />
	...
	<cffunction name="setDatasources" access="public" output="false" returntype="void">
		...
		<cfloop from="1" to="#arrayLen(sortedDSNs)#" index="i">
			...
			<cfif len(thisType)>
				<cfset objDatasource = createObject("component","model.datasource.datasource").init(dsns[thisDSN].name,thisType,dsns[thisDSN].driver) />
	...
	<cffunction name="getDatasource" access="public" output="false" returntype="model.datasource.datasource">
